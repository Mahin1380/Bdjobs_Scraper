...
_ABBREV_MAP = {
    ...
    'HSC': 'HSC', 'HIGHER SECONDARY CERTIFICATE': 'HSC',
}
...
def parse_edu(text: str) -> tuple:
    ...
    seen_degrees = []
    for match in _DEGREE_RE.finditer(str(text)):
    ...
    seen_majors = []
    for match in _MAJOR_RE.finditer(str(text)):
        matched = match.group(1)

        canonical_major = next(
            (m for m in _MAJORS if m.lower() == matched.lower()),
            matched
        )

        if canonical_major not in seen_majors:
            seen_majors.append(canonical_major)
...
def add_edu_columns(df: pd.DataFrame, source_col: str = 'eduRec') -> pd.DataFrame:
    df = df.drop(columns=['degree_level', 'major'], errors='ignore')

    parsed = df[source_col].apply(
        lambda x: pd.Series(
            parse_edu(x),
            index=['degree_level', 'major']
        )
    )
...
def _resolve_division(location: str) -> str:
    ...
    for key, division in _LOCATION_TO_DIVISION.items():
        if key in location:
...
def add_division_column(
    ...
    df.insert(
        insert_pos,
        'division',
        df[source_col].apply(_resolve_division)
    )
...
def add_experience_level_column(
    df: pd.DataFrame,
    source_col: str = 'experience'
) -> pd.DataFrame:

    df = df.drop(columns=['experience_level'], errors='ignore')
    ...
    df.insert(
        insert_pos,
        'experience_level',
        df[source_col].apply(_parse_experience_level)
    )
